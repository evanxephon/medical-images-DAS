...
选择X * Y大小的kernel，将kernel中选定的区域替换成其他记录同样区域的数据。这种方法同样可以生成大量数据。我们原本数据为一维的长度为136的数据，在kernal替换策略中，我们需先将数据转换成4 * 34的二维数据。在这里我们在不同year之间的数据替换可能能保存疾病的一种变化趋势。  

By choosing a fixed kernel with X * Y order for all original data cross different groups, we then substitute the entry of the kernel with other records from the subject of same group label. Repeat that process till every data has been covered. Apparently, we can induct that the size of generated data set will be linear by X, Y： N = (L-X+1) * (W-Y+1), where L=34, W=4 in our case.
